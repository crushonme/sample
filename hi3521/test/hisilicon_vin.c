/******************************************************************************
  Hisilicon HI3521 video input functions.

  Copyright (C), 2010-2011, Hisilicon Tech. Co., Ltd.
 ******************************************************************************
    Modification:  2014-10 Created
******************************************************************************/
#ifdef __cplusplus
#if __cplusplus
extern "C"{
...
HI_S32 NVP_CfgV(VIDEO_NORM_E enVideoMode,VI_WORK_MODE_E enWorkMode)
{
//TODO
	return 0;
}
...
*****************************************************************************/
HI_VOID Hisilicon_COMM_VI_SetMask(VI_DEV ViDev, VI_DEV_ATTR_S *pstDevAttr)
{
    switch (ViDev % 4)
    {
        case 0:
...
HI_S32 Hisilicon_COMM_VI_Mode2Param(SAMPLE_VI_MODE_E enViMode, SAMPLE_VI_PARAM_S *pstViParam)
{
    switch (enViMode)
    {
...
*****************************************************************************/
HI_S32 Hisilicon_COMM_VI_Mode2Size(SAMPLE_VI_MODE_E enViMode, VIDEO_NORM_E enNorm, RECT_S *pstCapRect, SIZE_S *pstDestSize)
{
    pstCapRect->s32X = 0;
    pstCapRect->s32Y = 0;
    switch (enViMode)
    {
		...
		case SAMPLE_VI_MODE_1_VGA:
		case SAMPLE_VI_MODE_TEST:
		    pstDestSize->u32Width = 640;
            pstDestSize->u32Height = 480;
            pstCapRect->u32Width = 640;
...
HI_S32 Hisilicon_COMM_VI_GetSubChnSize(VI_CHN ViChn_Sub, VIDEO_NORM_E enNorm, SIZE_S *pstSize)
{
    VI_CHN ViChn;
...
*****************************************************************************/
VI_DEV Hisilicon_COMM_VI_GetDev(SAMPLE_VI_MODE_E enViMode, VI_CHN ViChn)
{
    HI_S32 s32Ret, s32ChnPerDev;
    SAMPLE_VI_PARAM_S stViParam;

    s32Ret = Hisilicon_COMM_VI_Mode2Param(enViMode, &stViParam);
    ...
    SAMPLE_VI_MODE_E enViMode, SAMPLE_VI_CHN_SET_E enViChnSet)
{
    ...
    stChnAttr.bFilp = (VI_CHN_SET_FILP == enViChnSet)?HI_TRUE:HI_FALSE;
...
HI_S32 SAMPLE_COMM_VI_Start(SAMPLE_VI_MODE_E enViMode, VIDEO_NORM_E enNorm)
{
    ...
    for(i=0; i<stViParam.s32ViChnCnt; i++)
    {
        ...
        if (HI_TRUE == SAMPLE_COMM_VI_IsHd(enViMode))
        {
            ...
            s32Ret = SAMPLE_COMM_VI_GetSubChnSize(ViChn_Sub, enNorm, &stTargetSize);
            if (HI_SUCCESS != s32Ret)
            {
                SAMPLE_PRT("SAMPLE_COMM_VI_GetSubChnSize(%d) failed!\n", ViChn_Sub);
                return HI_FAILURE;
            }
            s32Ret = SAMPLE_COMM_VI_StartChn(ViChn_Sub, &stCapRect, &stTargetSize,enViMode, VI_CHN_SET_NORMAL);
            if (HI_SUCCESS != s32Ret)
            {
