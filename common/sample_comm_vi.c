...
VI_DEV_ATTR_S DEV_ATTR_RGB_VGA =
/* 典型时序3:VGA@60fps典型时序 (对接时序: 时序)*/
{
	/*接口模式*/
...
HI_S32 SAMPLE_NVP_CfgV(VIDEO_NORM_E enVideoMode,VI_WORK_MODE_E enWorkMode)
{
//TODO
	return 0;
}
...
HI_S32 SAMPLE_COMM_VI_ADStart(SAMPLE_VI_MODE_E enViMode, VIDEO_NORM_E enNorm)
{
    ...
    switch (enViMode)
    {
		...
		case SAMPLE_VI_MODE_TEST:
            ...
            if (s32Ret != HI_SUCCESS)
            {
                SAMPLE_PRT("SAMPLE_OV7725_CfgV failed with %#x!\n",\
                        s32Ret);
                return HI_FAILURE;
            }
			enWorkMode = VI_WORK_MODE_1Multiplex;
